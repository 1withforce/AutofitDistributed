...
class Distributor(asynchat.async_chat):
    ...
    def send_work(self, worker):
        ...
        obj = {'id': name, 'duration': 20,
               'files': [[APP_MD5, APP_URL, "modsquare.app"],
    ...
    def finished_work(self, successful, obj):
        ...
        try:
            ...
            #print "%s: %s Workunit Error" % (name, self.running[name])
...
def getInfo(data, name='data'):
    ...
    while i < len(data):
        ...
        if type(data[i]) == list or type(data[i])==numpy.ndarray:
            print data[i][:4],"..."
        else:
            print data[i]
        i+=1
...
def runAutofit(jobData):
    ...
    Distributor(find_server(), "modsquare", jobData)
